    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(user_email=form.email.data).first():
            ...
            new_user = User(
                ...
                user_password=hashed_salted_password,
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            flash("The password does not match the correct one! Please try again.")

        elif user and check_password_hash(user.user_password, password):
            login_user(user)
            return redirect(url_for('get_all_posts'))
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, current_user=current_user, is_edited=True)
